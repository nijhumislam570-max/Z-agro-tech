...
const AuthPage = () => {
  ...
  useEffect(() => {
    ...
    const loadOauthProviders = async () => {
      ...
      if (!supabaseUrl || !publishableKey) {
        if (!cancelled) {
          setOauthProviders({ loaded: true, google: false, apple: false });
        }
        return;
      }
      ...
      try {
        ...
        if (!cancelled) {
          setOauthProviders({
            ...
            apple: Boolean(body.external?.apple),
          });
        }
      } catch {
        if (!cancelled) {
  ...
  const showOauthSection =
    oauthProviders.loaded && (oauthProviders.google || oauthProviders.apple);
          ...
          <div className="backdrop-blur-xl bg-white/85 dark:bg-card/80 border border-white/40 rounded-2xl shadow-2xl p-5 sm:p-7 transition-all duration-200">
            ...
            {showOauthSection && (
              <>
                ...
                <div className={`grid gap-3 ${oauthProviders.google && oauthProviders.apple ? 'grid-cols-2' : 'grid-cols-1'}`}>
